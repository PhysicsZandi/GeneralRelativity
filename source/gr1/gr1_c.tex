...
\section{Standard model of cosmology}
    ...
    The solutions of the Friedmann equations shows that the Universe must have started from a singularity at $a = 0$ and expands from that ever since. This is the basis of the Hot Big Bang model. Its main adversary was the steady model, in which energy density remains constant. However, the Cosmic Microwave Background radiation shows that the Universe is cooling down. 

    The CMB radiation is the almost relic radiation, generated by the decoupled of radiation from matter on the last scattering surface. Before, photons were in thermal equilibrium with electrons and protons, and the only way to understand the physics of that period is through gravitational waves. The light we receive from different direction in the sky should not have the same temperature, because they were not causally connected. 

    Suppose we place ourselves at $r = 0$ and $t=0$. We want to find the proper distance that the photon has travelled from $r= r_s$ and $t=-t_s$ to us
    \begin{equation*}
        R \sim a(-t_s) r_s ~.
    \end{equation*}
    ...
    \begin{equation*}
        a(t) \sim t^\alpha ~, \quad r_s \sim \int_{-t_s}^0 dt ~ t^{- \alpha} \sim t_s^{1 - \alpha} ~,
    \end{equation*}
    where $\alpha = 2 / 3$ and $\alpha = 1 / 2$. Therefore, the proper distance is given by
    \begin{equation*}
    ...
    \end{equation*}
    This can explain CMB homogeneity: the Universe started so small to have all particles in causal connection and, then, there was an inflation phase of rapid expansion. Finally, it ended.
